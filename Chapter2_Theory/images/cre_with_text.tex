\begin{figure}[ht]
    ...
    \end{tikzpicture}
    \caption{The global mean annual \acrfull{cre} is the difference between the radiative components of the clear-sky \textcolor{red}{(cloud-free)} and all-sky \textcolor{red}{(cloudy)} radiative components. A positive sign can be describes a warming effect and negative a cooling, units in $W m^{-2}$. Inspired by Figure 15 in \citeauthor{Wild2019TheModels} \textcolor{red}{(\citeyear{Wild2019TheModels})}. \textbf{Trude:} I wrote latent heat, Wild used evaporation. Aren't we interested in the heat flux assisiated with evaporation which is latent heat? Det er lett å bytte tilbake hva blir mest riktig..?}
    \label{fig:cre}
\end{figure}