...
In this study a \acrshort{ar}-model is composed of 13041 individual regression models, one for each grid box. Four hyperparameters are available for tuning, these are feature scaling of the predictors, the inclusion of bias, number of lags and a potensial inclusion of environmental variables. Varying combinations of these parameters results in the set of models trained in this study. For more theoretical details please see Section \ref{sec:ARmodels}. 
...
The transformation is done by applying Equation \eqref{eq:scaling_data} to the predictors, one by one. The resulting data has a reshaped distribution, in the direction of a standard normal distribution with zero mean and unit variance. The feature scaling offers an additional benefit of increased numerical stability. %When applied the predictors is transformed according to the following Equation \ref{eq:scaling_data}. 
%The feature scaling is applied after the partitioning into training and test portions. 

It is important to perform the transformation after the data is split into training and test. The mean and standard deviation should be computed based on the training set, and applied to both sets. This is necessary to avoid a information leak between the test data and the trained model. A consequence of sneak peaking is an  unrepresentative measure on performance.
...
The naming conventions for the \acrshort{ar}-models used in this study $AR_{TB_L}$ or $TR_{TB_L}$. $AR$ or $TR$ describe whether the environmental variables are included in the dataset of not. $TR$ is short for traditional and represent that environmental variables is omitted. \acrshort{ar} represent the opposite, their inclusion. B represent bias, T symbolize that scaling predictors is applied and $L_x$ reveals the number of lags, here $x$.  
...
The hyperparameters bias and transformation of the predictors is mutually exclusive. It both were included the transformation could cancel the effect of the bias when subtracting the mean. 
\begin{table}[h]
    \centering
    \resizebox{\textwidth}{!}{%
    \begin{tabular}{ccccc}
...
The models are all evaluated using \acrfull{mae}.
Recall that \acrshort{ar}-models are optimized for fitting the next timestep, not longer sequences. Figure \ref{fig:heatmap_ar_models} show the performance of all \acrshort{ar}-models included in this study, varying the number of lags on the first axis and the other hyperparameters on the second axis. The score is computed computed by using Equation \eqref{eq:mae}, and setting the parameters $m = 81$, $n=161$ and $k=43824$. \begin{figure}
    \centering
...
With the exception of the $AR-T$-configuration, most models increase most in performance when adding the first lag. After adding the first lag, the performance increase, but not very much. This clearly states that the cloud cover at previous timesteps are a useful predictor. The largest variations in the figures arise from different configurations of $AR$, $B$, $T$ and $TR$. 

%%%%%%%%%%%%%%%%%%%%% Ta for deg alle variablene. 
Employing $T$, doesn't appear to be a good predictor. A grid \acrshort{mae} of roughly $0.5$ is quite a lot when the target varies in the range from 0 to 1. Addding a bias to $AR$ improves the performance, for $TR$ it has the oposite effect and decrease the performance.

Second to last is $TR$, does it a lot better than $T$, and the set of models lie in the area of $0.14$, but by only employing cloud cover might prove to give to little information. Since $AR$ outperforms $TR$ for all other configurations, it is clear that he environmental variables provide useful information. 

The performance of the best model, $AR-B-L_5$ of $0.04901$ is great. Showing that there is enough information in the set of environmental variables and previous cloud cover to predict at least on hour into the future. 
...
\end{figure}
Figure \ref{fig:weights_best_model} shows the grids of the weights in $AR-B-L5$. The colorbars are different for all subplots, but the colors, red being positive and blue being negative is consistent. The \acrshort{ar}-model is a weighted sum over all the variables. Negative values for any of the input variables would be unphysical. Positive values can therefore be interpreted as cloud forming and negative as dissipating?

By studying the weights of the five hours of previous timesteps its clear that they all contribute in producing cloud cover, there $L_1$ clearly have the highest weights. Close to one for the entire grid. The minor change for increasing number of lags, shown in Figure \ref{fig:heatmap_ar_models}, can most likely be attributed to the low magnitude of the weight. 
%\textbf{This explains the copying effect seen in the sequence plotted.}

Surface pressure is negative for the entire grid, this is consistent with the physics stating that higher pressure, are associated with ascending air masses, and they don't produce cloud cover.

The behavior of relative and specific humidity is puzzling, the exhibit the approximately opposite pattern. In most regions where relative humidity produce clouds, specific humidity reduce it cloud cover. In regions over Spain the same appears to be going on with temperature and specific humidity. 
