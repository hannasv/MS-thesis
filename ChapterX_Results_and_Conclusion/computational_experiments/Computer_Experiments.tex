...
The numerical methods used in this study are described in Chapter \ref{ch:num_methods}. The code is available on GitHub in the project repository named ``MS'' on \href{https://github.com/hannasv/MS}{https://github.com/hannasv/MS}. Instructions for downloading reanalysis (ERA5) data using python is provided. Due to licence requirements on the \acrshort{msg} data, no part of \acrshort{ecc} is made available for downloading. The reader need to aquire their own \acrshort{msg} data throught the eumetcast portal and can regridd the product using the code made availble.
...
The following sections contain the configurations of the models compiled for this study along with descriptions of hyperparameters. A model is compiled based on a choice of hyperparameters. It is a set of decisions made prior to training, as mentioned in Section \ref{sec:artificial neural networks}. In the search for the best model configuration, different combinations of hyperparameters are evaluated. This is done manually since the choice of architecture easily overload the system memory resources.
...
The partitioning into training, validation and test data is different for the models. \acrshort{ar}-models are computed based on an analytical solution, consequently there is no need for validation data. The \acrshort{convlstm} is a numerical solution, continuously evaluated against the validation dataset and adjusted to reduce the loss. This is all mentioned in Section \ref{??}. The weight resience information of the validation dataset indirectly via the adjustments made and this raise the need for a third dataportion the model can be individually evaluated on.

Both models are tested on 2014 to 2018. The \acrshort{ar}-model is trained on the period 2004 to 2013, while the \acrshort{convlstm} is trained on  2004 to 2011 and validated on 2012 to 2013. 

The test period is chosen to be the most resent, based on the assumption that it is most representative for the near future. 

Another small difference in the input volumes to the models, is the handling of missing data. Samples containing gaps are removed for the \acrshort{ar}-models and for \acrshort{convlstm} it is replaced by the out-of-sample value, $c=1.5$. 
...
The downloaded size of \acrshort{ecc} is 17Gb, stored in \acrshort{netcdf}-files. Comarisons to the works by  by \citeauthor{precip_nowcasting} (\citeyear{precip_nowcasting}) and \citeauthor{SunAirLSTM} (\citeyear{SunAirLSTM}), need to by done on the basis of input volumes. \textbf{Add the input volume}

A method to successfully train deeper models is to reduce the spatial or temporal resolution. This was applied in ``precipitation nowcasting'', arguing that it reduce noise, the make the reduction from 300x300 pixels to 100x100. As mentioned in Section \ref{sec:cloud_in_climate_system}, cloud cover has an average lifetime of one hour, and the mentioned approach is not applicable to this task. It would most likely produce a significant loss of information.
...
standard scaling the predictors (T), transforming the target (S), the inclusion of intercept (B), order of the models (L) and environmental variables (AR vs. TR). Varying combinations of these parameters results in the set of models used in this part of the experiments. 


A summary of the performance of the calibrated models is provided in Figure \ref{fig:results_ar_models}. Keep in mind that one \acrshort{ar}-models is the aggregation of 13041 individual regression models. The displayed score is an average over the entire grid, resulting in the average pixels score.
...
The environmental variables never appear in isolation. Lags describe the number of previos timesteps of fractional cloud cover included as a predictor. For example, if lags is one, $L=1$, then the CFC of the previous hour is included along with eviornmental variable for the current hour.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% TRUDE RETTET OVER HER

\subsubsection{Experimental setup (AR-models)}
\textbf{Move naming convention to the introductioon of ar models?}
The naming conventions used $AR_{STB_L}$ or $TR_{STB_L}$. B stands for bias, T for scaling predictors, S for sigmoid transformation of the target and L for the number of lags, previous timestep of cloud cover. AR og Tr describe whether the environmental variables are included in the dataset of not. AR denotes that it is includen and TR denotes that it is omitted.

%This description elaborated in Table \ref{tab:ar_model_config}.
To make it easier to understand the naming convention used, Table \ref{tab:ar_model_config} provide three examples. Applying the following convention $\times$ denoted not applied, \checked denotes applied.
...
\begin{figure}
    \centering
    \includegraphics[scale = 0.5]{python_figs/MSE_score_AR_models.png} %\textbf{same colormap as MAE or obly for grids} 
    \caption{DUMMY DATA MSE score ar models.}
    \label{fig:results_ar_models}
\end{figure}

Figure \ref{fig:results_ar_models} is computed using Equations \eqref{eq:mse} and \eqref{eq:mae}, and setting the parameters $m = 81$, $n=161$ and $t=\text{hours in testset}$.
\begin{figure}
    \centering
...
Figure \ref{fig:grid_mse_best_model} show the score of each pixel in the best model. Table \ref{tab:weights_best_model} illustrates the best weights 
\begin{table}[hp]
    \centering
    \resizebox{\textwidth}{!}{%
    \begin{tabular}{cccccc}
...
The formulation of the air quality forecasting problem presented by \citeauthor{SunAirLSTM} is similar to the formulation of the cloud fractional cover prediction problem presented in this project. 
...
A lot of architectural decisions such as batch size, sequence length, number of filters and number of layers can cause the \acrshort{gpu} to run out of memory. In these cases the compiled model is not trainable. 

Between each \acrshort{convlstm}-layer there is aBatch Normalization layer. According to the \citepaper{ioffe2015batch} this should simplify the training process. 

The list of hyperparameter is long, consequently a subset of them is tuned in this study and the rest is kept constant.

The following section describe the tuned portion. The dataset is partitioned into subsets called batches. The batch size is the number of samples a weight update is based on. Epochs is the number of times the model loops over the entire dataset. The sequence length is the number of timestamps a model aims to learn to predict. Number of hidden states it the number of filters it learns in each layer. Cite Section \ref{sec:convolutional neural network} for description of hidden states. 
...
This section describe the hyperparameters kept constant.
Padding same is applied to all layers, to make sure the input and output dimensions are the same. This is described in more detail in Section \ref{sec:padding}. The model returns a sequence. The input sequences are not shuffled. The output filter and kernal size is one to concatenate all the previos hidden states to one without altering the output dimension. This is necesary for the predictions made.

The weights are initilized based on the scheme ``LeCunn uniform''. \textbf{cite paper} Callbacks such as early stopping with patience of 10 epochs and terminate on nans have been applied to avoid prolonged training time. The optimizer ADAM is used with default settings. The loss is means squared error function and MAE is used to evaluate the models. MSE penalize points further away, but it is harder to interpret. 
...
%\begin{enumerate}
    ...
    %\item return sequence = True % False defies the purpose if the LSTM 
...
Models are given names based on an extension of the convention from \citepaper{precip_nowcasting}. The paper introduces the convention ConvLSTM-hidden states-filter$\times$filter, adding the batch size and sequence length as tunable parameters. The resulting naming convention is \newline $ConvLSTM-B_{x}-SL_{y}-hidden states-filter$\times$filter$. Table \ref{tab:convlstm_config} provide a set of example to 

\begin{table}[hp]
    \centering
    \resizebox{\textwidth}{!}{%
    \begin{tabular}{ccccc}
     & \textbf{Sequence Length} & \textbf{Batch Size} & \textbf{Hidden States} & \textbf{Kernels} \\ \hline
    \textbf{$ConvLSTM-B_{5}-SL_{24}$ \newline $-256-3\times3-256-3\times3$} & 24 & 5 & [256, 256]   & [3, 3] \\ \hline
    \textbf{$ConvLSTM-B_{15}-SL_{24}-256-5\times5-256-5\times5$} & 24 & 15 & [256, 256]  & [5, 5] \\ \hline
    \textbf{$ConvLSTM-B_{10}-SL_{24}-128-1\times1-128-1\times1$} & 24 & 10 &  [128, 128] & [1, 1] \\ \hline
    \textbf{$ConvLSTM-B_{5}-SL_{6}-64-3\times3-64-3\times3$} & 6 & 5 & [64, 64] & [3, 3] \\ \hline
    \textbf{$ConvLSTM-B_{5}-SL_{6}-256-3\times3-256-3\times3$} & 6 & 5 & [64, 32] & [3, 3] \\ \hline
    \textbf{$ConvLSTM-B_{5}-SL_{6}-8-3\times3-8-3\times3-8-3\times3$} & 6 & 5 & [8, 8, 8] & [3, 3, 3] \\ \hline
    \end{tabular}%
    }
    \caption{Examples to simplify the explanation of configurations and models. \textbf{Remove num. params. should be in a separate table containing m}}
    \label{tab:convlstm_config}
\end{table}

Figure \ref{fig:convlstm_loss} show the loss curves in the training prosess for all the compiled models in this study. The training loss is higher than the validation loss since there is a higher number of samples in the training set than the validation set and it is not scaled.
...
\textbf{Make table containing all losses.}

\begin{figure}
    \centering
    \input{ChapterX_Results_and_Conclusion/computational_experiments/inputvolume.tikz}
...
The best performing \acrshort{convlstm}-model is the $ConvLSTM-B_{10}-SL_{24}-16-3\times3-16-3 times3$. It architecture is shown in Figure \ref{fig:best_ml_architecture}, the input volume is currently very simplified. The real input volume is provided in Figure \ref{fig:input_volume_conv_lstm}. \textbf{Adjust both of them, mayby add a opach blu box illustrating a batch. }

\begin{figure}
    \centering
    \input{ChapterX_Results_and_Conclusion/computational_experiments/inner_structure_best_model.tikz}
    \caption{Structure of cloud cover forecasting model. \textbf{Add tekst} Four input variables, 16 hidden states in each convolutional LSTM layer and $3\times 3 kernel$. \textbf{Help how to best include the dimension sequence length and batch size}. The input shape is 10x24x81x161x4 and output 24x81x161x1}
...
\begin{enumerate}
    ...
    \item Check number of prediction below 0
\end{enumerate}
