\documentclass{MSthesis} 
% subclass of the document class report.
\pgfplotsset{compat=1.16}

%\def\checkmark{\tikz\fill[scale=0.4](0,.35) -- (.25,0) -- (1,.7) -- (.25,.15) -- cycle;} 
\def\scalecheck{\resizebox{\widthof{\checkmark}*\ratio{\widthof{x}}{\widthof{\normalsize x}}}{!}{\checkmark}}
%that's defined it - now for a test

\begin{document}

% Formatting titles and spaces 

% Originally its
% Chapter 1:
% Name of chapter one
\titleformat{\chapter}[hang] 
{\normalfont\huge\bfseries}{\thechapter}{1em}{}  

% left margin, vertical space,  space to section, 
\titlespacing*{\chapter}{0cm}{0cm}{0cm}
\titlespacing*{\section}{0cm}{0cm}{0cm}
\titlespacing*{\subsection}{0cm}{0cm}{0cm}
\titlespacing*{\subsubsection}{0cm}{0cm}{0cm}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% USING THE TWOSIDED OPTION left = inner , and right = outer
\myfrontpage
\mycopyright
\newgeometry{top=2.5cm, bottom=2.5cm, left=3.0cm, right=2.5cm}
%\newpage\null\thispagestyle{empty}\newpage

% Abstract should have first page nr,
\pagenumbering{roman}
\input{Mics/abstract.tex}
\input{Mics/acknowledgement.tex}

\tableofcontents

% Clear double page --> New chapters or listings begin on odd numbered pages.
\cleardoublepage

\listoffigures
\addcontentsline{toc}{chapter}{\listfigurename}
\cleardoublepage

% List of tables 
\listoftables
\addcontentsline{toc}{chapter}{\listtablename} 
% add list of tables to contents line
\cleardoublepage

% List of acronyms
\input{acronyms.tex}
\cleardoublepage

\pagenumbering{arabic} % page numbering to arabic
\input{./Chapter1_Intro/Introduction}
\cleardoublepage

\input{./Chapter2_Theory/Theory}
\cleardoublepage

\input{./Chapter3_Method/Method}
\cleardoublepage

\input{./ChapterX_Results_and_Conclusion/res_and_conc.tex}
\cleardoublepage

%\input{./Chapter4_Dataset/Dataset}
%\cleardoublepage

%\input{Chapter5_Experiments/Computer_Experiments}
%\cleardoublepage

%\input{./Chapter6_Discussion/discussion.tex}
%\cleardoublepage

\input{./Chapter7_Conclusion/conclusion.tex}
\cleardoublepage

\input{./Appendix/Appendix}
\cleardoublepage

\addcontentsline{toc}{chapter}{Bibliography}
\printbibliography
\cleardoublepage

\end{document}
