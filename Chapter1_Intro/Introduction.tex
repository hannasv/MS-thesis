...
simulations prepared for the latest assessment report (\acrshort{ar5}) produced by the \acrfull{ipcc} puts \acrshort{ecs}s in the range from $2.1^oC$ to $4.7^oC$ (\cite{IPCC_CH9_climate_models}, pp.817). These runs are forced with instantaneous quadrupling of the $CO_2$ concentration %(of pre-industrial levels 
...
Including the effects of clouds on large scale processes, in climate models, has proven difficult \textit{There is very high confidence that uncertainties in cloud processes explain much of the spread in modelled climate sensitivity.  } (\cite{IPCC_CH9_climate_models}). %TS: Biten ovenfor (om at usikkerhet knyttet til skyer forklarer mye av usikkerheten i ECS) må være med, ellers blir ikke linken til arbeidet ditt tydelig nok
