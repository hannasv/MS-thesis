...
The following sections contain the configurations of the two methods used in the experimental setup used in this project, along with description of tunable parameters. Automatic hyper parametertuning is used for the complicated \acrshort{convlstm} models, while the simpler \acrshort{ar}-models, are manually tuned. This is done based on convenience, the parameter space is a lot larger for \acrshort{convlstm} and software for automatic optimixation is provided by Chollet.
For a small parameter space, manual optimization allows you to become more failiaar with a particular model. 
...
Feature scaling is used to normalize or scale the predictor variables, this can be done to avoid possible problems related to numerical errors. The predictors was normalized according to the following Equation \ref{eq:scaling_data}. The feature scaling is applied after the partition into training and test dataset. 
\begin{equation} \label{eq:scaling_data}
    \mathbf{x} = \frac{\mathbf{x} - \bar{\mathbf{x}}}{\sigma_{\mathbf{x}}}
\end{equation}
where $\bar{\mathbf{x}}$ is the average and $\sigma$ is the standard deviation computed based on the respective predictor in the trainingset. 

The mean and standard deviation is applied to the test dataset. If the statistics of the test dataset would have been included, the model would have a sneak peaked at the test data and the test score would not be general.

\subsubsection{Transforming target} \label{sec:transforming_target}
Transformation of data us useful for avoiding predicting unphysical values, the reverse/ inverse transformation truncates the values in the range or [0, 1].
...
The machine learning experimental setup is adopted from the paper \citepaper{SunAirLSTM}. The model names is given following this structure. most easily described by a example, model \textit{ConvLSTM 3x3-256-2}, is a \acrshort{convlstm} model using a $3\times 3$ filter, 256 hidden states and has 2 layers. 

All models use the MSE loss in combination with the Adam optimiser. The list describes models tested in this experiment. 
...
optimizer = A
̈́
\subsubsection{Automatic hyperparameteroptimization}  \label{sec:hyperparam_tuning}
ML is more complicated and would further benefit 
...
% De viste at den var bedre enn FC-LSTM for fully connected 
Convolution LSTM was introduced for applications to precipitation nowcasting. Similar to traditional models the predictors is the target variable at earlier time steps. 
...
In the mind-boggling amount of choices for hyperparameters, the initial configuration used in the conducted experiments for this project draw inspiration from this paper. \textbf{Kjør eksperimentene før du fyller i resten av denne seksjsonen}
