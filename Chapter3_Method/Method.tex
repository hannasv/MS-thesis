...
To asses model performance you need to do this based on a criterion, this is naturally 

In order to acquire a certain skill you need a measure determining how close you are. 
Use the sum of square or absolute values in order to not penalise point on the lower side of the line. Or not having to deal with negative distances. 
...
Deep learning is a relatively young field. \acrshort{convlstm} is a relatively new model, most machine learning problems don't involve both space and time dimension, and is purely classification task. This explains why there is so litle litterature on this topic. 

To main works done by \cite{precip_nowcasting}, who developed this model, and \cite{SunAirLSTM} who applied it to the air quiality forecasting problem, quite similar to our own task. 
