...
Model evaluation is done based on criteria, also known as metrics. The above mentioned models output volumes, and the metrics are written in their three-dimensional form.
...
\end{equation} 
where mean value of $\hat{y}$ is defined as $\bar{y} =  \sum_{j=0}^{m-1}\sum_{k=0}^{o-1}y_{i, j, k}^2$. $R^2$ describes how much of the variation in the dataset you are able to capture with your model. The absolute value or sum of squares is to avoid penalising points on the lower side of the line, it has an additional benefit of not having to deal with negative distances. 
...
Deep learning is a relatively young field and \acrshort{convlstm} even younger model, developed in 2015. Few experiments are conducted and there is articles on the topic. Most machine learning works are done on task of classification.

The model was developed by \citepaper{precip_nowcasting}, for the task of precipitation nowcasting, and \citepaper{SunAirLSTM} applied it to the air quality forecasting problem. 

\citepaper{precip_nowcasting} use only gridded data and the volume of data is $100\times 100$ after resizing the filters to remove noise. The radar echo dataset have traning samples (8148), validation (2037), test (2037). \textbf{Should i reduce my training dataset.} Each sample consist of 20 frames, 5 used for input and 15 used for prediction. No metrics usefull for comparison.

\citepaper{SunAirLSTM} use both gridded data and point measurements. The weather data volume for each timestep is $(21 \times 31 \times 5)$. 16 months of hourly data divided into sequences of 72 frames, 24 for input and 48 for frames. 


Batch size of one will allow you to build a deeper model, but it takes longer to train. Have a mse, r2, num parameters i en tabell når du legger frem resultatene. 
%Basert på figuren ser det ut som batch sizes er 1, det må ta lang tid å kjøre.

\begin{enumerate}
    \item learning rate scheduler to 0.001 after 5 epochs
    \item gradient clipping
    \item filters of 1, 3, 5
    \item hidden states 256, 128
    \item 2 or 3 layers
    \item optimised to predict hat it gives to outputs. 
    \item future work ResNet + Rnn -- noen som har gjort noe på dette?
    \item kan være verd et forsøk å ha batch size på en for å få sjekket ut higher level abstractions.
\end{enumerate}
