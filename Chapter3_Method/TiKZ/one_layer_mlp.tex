...
\end{tikzpicture}
\caption{Neural network with one hidden layer. The connections between the layers are the weights. The input layer has four units, the hidden layer has five and the output layer has three. In total there are 35 parameters in this network (37 if bias is included). The sketch is based on the example \cite{ffnn}.}
