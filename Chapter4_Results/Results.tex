...
\subsection{Testing regridding routine}
...
Unless stated otherwise a model is trained on data in the period 2004 to 2013 and tested on 2014 to 2018. 
...
\textbf{Make bar plot showing the average number of training and test samples for AR models. This might explain the perforamce of certain models. }

\subsection{ERA5}
Performance of ERA5 model compared to the satellite. 
\textbf{Make table from script running on wessel}
...
\begin{table}[h]
    ...
    \hline
    \multicolumn{1}{|c|}{} & \textbf{Scaling predictors} & \textbf{Transforming target} & \textbf{Order} & \textbf{Environmental variables} \\ \hline
    \textbf{Model 1} & \surd & \surd & 1 & \surd \\ \hline
    \textbf{Model 2} & \surd & \surd & 0 & $\times$ \\ \hline
    \textbf{Model 3} & \surd & \surd & 1 & $\times$ \\ \hline
    \textbf{Model 4} & \surd & \surd & 2 & $\times$   \\ \hline
    \textbf{Model 5} & \surd & \surd & 3 & $\times$ \\ \hline
    \textbf{Model 6} & \surd & \surd & 4 & $\times$ \\ \hline
    \end{tabular}
    }
    \caption{Table configuration of models. $\times$ denoted not applied, \surd denotes applied}
...
\subsubsection{Crossvalidation - best model for three different test sets}
% Prøv å gjøre dette plottet ikke en hel side da er det lettere for latex å plassere det tror jeg.
\textbf{Include this if you have the time}
Kryssvalidering.
...
All models use the MSE loss in combination with the Adam optimizer. The list decribes models tested in this experiment. 

\begin{enumerate}
    \item ConvLSTM 1x1-256-2
    \item ConvLSTM 3x3-256-2
    \iten ConvLSTM 5x5-256-2
...
\input{Chapter4_Results/figurs/estimating_changes_lat_lon}